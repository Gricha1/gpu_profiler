...
import asyncio, hashlib, hmac, os, subprocess, time
...
def _embedded(): return bool(getattr(app.state, "embedded", False))
...
def _auth(r):
    if _embedded():
        ...
        user = user_config.user_for_ip(client)
...
def _command(args, timeout=120):
    ...
    if result.returncode: raise RuntimeError(output or f'exit {result.returncode}')
    return output

@app.post('/api/control/update')
async def update(request:Request):
    if _embedded(): raise HTTPException(404, 'control is disabled on the embedded UI')
    if not _auth(request): raise HTTPException(401,'Требуется вход администратора')
    try:
        output=await asyncio.to_thread(_command,['git','-c','safe.directory=/host-repo','-C','/host-repo','pull','--ff-only','origin','main'])
        return {'ok':True,'message':output or 'Уже актуально'}
